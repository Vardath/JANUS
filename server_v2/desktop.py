...
def _runtime_payload(account_id: int) -> dict[str, Any]:
    ...
    for name, core in cores.items():
        ...
        core["fano"] = topology.metadata(name)
    return rt
...
def runtime_cores(authorization: Optional[str] = Header(default=None), username: str | None = None):
    ...
    return {"profile":a["username"],"architecture":rt["architecture"],"runtime":rt}


@router.get("/desktop/cores")
def cores(authorization: Optional[str] = Header(default=None), username: str | None = None):
    a=_account(authorization); rt=_runtime_payload(int(a["id"]))
    return {"profile":a["username"],"architecture":rt["architecture"],"cores":list(rt["cores"].values()),"fano_lines":topology.FANO_LINES}
...
def core_observe(
    ...
    if core != "all": where += " AND core_name=?"; args.append(core)
    args.append(limit)
...
def home(authorization:Optional[str]=Header(default=None),username:str|None=None):
    ...
    return {
        "profile":a["username"],"status":"Active" if rt["phase"]=="wake" else "Dormant","architecture":"11-core",
        "unread_messages":int(unread["n"] if unread else 0),"latest_activity":dict(latest) if latest else None,
        "background_interval_minutes":15,"core_phase":rt["phase"],"core_runtime":rt,"external_api_budget_used_by_core_cycle":0,"messaging_action":True,
    }
