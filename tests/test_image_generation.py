...
def test_explicit_image_intent_and_visual_marker():
    with tempfile.TemporaryDirectory() as td:
        _, mod = _fresh(Path(td))
        assert mod.explicit_image_request("Please generate a picture of the topology")
        assert mod.explicit_image_request("Make me an explanatory diagram")
        assert not mod.explicit_image_request("Explain the topology in words")
        clean, prompt = mod.extract_visual_nomination("Text first. [[JANUS_VISUAL: a clear Fano plane diagram]]")
        assert clean == "Text first."
        assert prompt == "a clear Fano plane diagram"


def test_automatic_budget_stops_repeated_background_renders():
    with tempfile.TemporaryDirectory() as td:
        ...
        ok, reason = mod._budget_check(int(account["id"]), "auto", int(time.time()))
        assert not ok
        assert "automatic-image" in reason


def test_same_prompt_is_cacheable_without_another_render():
    with tempfile.TemporaryDirectory() as td:
        auth, mod = _fresh(Path(td))
        account = _account(auth)
        ...
        assert cached["file_id"] == stored["file_id"]


def test_background_multicore_rendering_remains_disabled_by_policy():
    with tempfile.TemporaryDirectory() as td:
        _, mod = _fresh(Path(td))
        assert "Multi-core autonomous visual deliberation/render loops remain disabled" in (mod.__doc__ or "")
        assert "Do not use visual generation for routine chat" in mod.VISUAL_POLICY
