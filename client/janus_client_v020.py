...
APP_NAME='JANUS - Global 7-3-1 v0.20';SERVER='https://janus-global-core.onrender.com';CFG=os.path.join(os.path.expanduser('~'),'.janus','client.json')
class API:
 def call(self,method,path,payload=None,timeout=120):
  data=None if payload is None else json.dumps(payload).encode();q=request.Request(SERVER+path,data=data,headers={'Content-Type':'application/json','Accept':'application/json'},method=method)
  try:
...
def save_cfg(x):os.makedirs(os.path.dirname(CFG),exist_ok=True);open(CFG,'w',encoding='utf8').write(json.dumps(x,indent=2))
class App(tk.Tk):
 def __init__(self):
  super().__init__();self.api=API();self.cfg=load_cfg();self.user=self.cfg.get('profile_id') or os.environ.get('USERNAME') or 'local-user';self.pages={};self.rows={};self.timer=None;self.auto=tk.BooleanVar(value=self.cfg.get('auto_refresh',True));self.seconds=tk.IntVar(value=int(self.cfg.get('refresh_seconds',10)));self.title(APP_NAME);self.geometry('1180x760');self.minsize(940,620)
  top=ttk.Frame(self,padding=10);top.pack(fill='x');ttk.Label(top,text='JANUS',font=('Segoe UI',18,'bold')).pack(side='left');ttk.Label(top,text='Global 7→3→1').pack(side='left',padx=8);self.status=tk.StringVar(value='connecting…');ttk.Label(top,textvariable=self.status).pack(side='right');body=ttk.Frame(self);body.pack(fill='both',expand=True);nav=ttk.Frame(body,padding=8,width=175);nav.pack(side='left',fill='y');nav.pack_propagate(False);self.content=ttk.Frame(body,padding=12);self.content.pack(side='left',fill='both',expand=True)
 ...
 def page(self,k):f=ttk.Frame(self.content);self.pages[k]=f;return f
 def head(self,p,title,sub=''):ttk.Label(p,text=title,font=('Segoe UI',18,'bold')).pack(anchor='w');ttk.Label(p,text=sub).pack(anchor='w',pady=(0,10)) if sub else None
 def build_home(self):p=self.page('home');self.head(p,'Home','Persistent JANUS status');self.home=tk.Text(p,state='disabled',wrap='word',font=('Segoe UI',12),padx=10,pady=10);self.home.pack(fill='both',expand=True)
 def build_chat(self):
 ...
 def build_list(self,k,title,sub):
  ...
  if k=='messages':bar=ttk.Frame(p);bar.pack(fill='x',pady=6);ttk.Button(bar,text='Mark read',command=lambda:self.message_state('read')).pack(side='left');ttk.Button(bar,text='Dismiss',command=lambda:self.message_state('dismissed')).pack(side='left',padx=6)
 def build_cores(self):p=self.page('cores');self.head(p,'Cores','7 specialist lenses → 3 synthesis bridges → 1 JANUS voice');self.cores=tk.Text(p,state='disabled',wrap='word',font=('Segoe UI',11));self.cores.pack(fill='both',expand=True)
 ...
 def send(self):
  ...
  if not m:return
  self.entry.delete('1.0','end');self.say('You',m);self.status.set('thinking…');self.bg(lambda:self.api.call('POST','/desktop/chat',{'profile_id':self.user,'message':m}),lambda r:(self.say('JANUS',r.get('reply','')),self.status.set('online')))
 def show(self,k):
 ...
 def render_list(self,k,r):
  ...
  for i,x in enumerate(items):typ=x.get('event_type') or x.get('role') or 'item';typ=('NEW · ' if x.get('state')=='unread' else '')+typ;txt=x.get('detail') or x.get('content') or '';tree.insert('','end',iid=str(i),values=(x.get('created_at','')[:19].replace('T',' '),typ,txt.replace('\n',' ')[:180]))
 def render_cores(self,r):self.settext(self.cores,'1 INTEGRATOR\n'+(r.get('one_integrator') or {}).get('description','')+'\n\n3 BRIDGES\n'+''.join(f'• {k}: {v}\n' for k,v in (r.get('three_bridges') or {}).items())+'\n7 LENSES\n'+''.join(f'• {k}: {v}\n' for k,v in (r.get('seven_roles') or {}).items()))
 ...
 def detail(self,k):
  sel=getattr(self,k+'_tree').selection()
  if sel:x=self.rows[k][int(sel[0])];messagebox.showinfo(k.title(),x.get('detail') or x.get('content') or json.dumps(x,indent=2))
 def message_state(self,state):
  sel=self.messages_tree.selection()
  if sel:x=self.rows['messages'][int(sel[0])];self.bg(lambda:self.api.call('POST',f"/desktop/messages/{x['id']}/state",{'profile_id':self.user,'state':state}),lambda r:self.refresh('messages'))
 ...
 def bg(self,fn,done):
  def run():
   ...
   except Exception as e:self.after(0,lambda err=str(e):self.status.set('error: '+err[:100]))
