...
def _foreground_notes(profile,message):
 """Mandatory foreground society pass. Returns externalizable notes, never private CoT."""
 try:
...
def install(app):
 ...
 async def desktop_chat_interface(payload:dict[str,Any]):
  ...
  # CRITICAL ORDER: question -> 7 specialists -> hemispheres -> consensus -> interface -> response model.
  # The response model is forbidden from inventing a parallel answer and appending telemetry afterward.
  deliberation,notes=await asyncio.to_thread(_foreground_notes,profile,message)
  runtime=janus_sleep_cycle.status(); history=_recent_context(profile)
  if not os.environ.get('OPENAI_API_KEY'):
   ...
   instructions=JANUS_SELF_KNOWLEDGE+'''\n\nJANUS INTERFACE CONTRACT:\nYou are the final Interface of an 11-core 7->2->1->1 deliberation. The CORE DELIBERATION below is PRIMARY. Answer the user's actual question from its substantive findings. Do not independently write a generic assistant answer and then append runtime telemetry. Do not lead with architecture, cycle counts, Fano counters, or process descriptions unless the user explicitly asks diagnostics. Surface useful conclusions, surprising connections, genuine disagreements, hypotheses, evidence gaps, and worthwhile next questions found by the cores. Preserve uncertainty and distinguish retained memory, model consultation, web evidence, and inference. Never expose or claim private chain-of-thought; these are externalizable core summaries. If core material is thin, say so rather than pretending depth.''' 
   inp='CORE DELIBERATION (externalizable summaries):\n'+json.dumps(notes,ensure_ascii=False)+"\n\nFOREGROUND RESEARCH STATUS:\n"+json.dumps(deliberation,ensure_ascii=False)+"\n\nSERVER STATE (secondary diagnostic context):\n"+_live_runtime_evidence(runtime,profile)+(f"\n\nRecent retained conversation:\n{history}" if history else '')+f"\n\nUSER QUESTION:\n{message}"
   try:
    async def call():
  ...
  result={'reply':reply,'profile':profile,'mode':'core_deliberation_primary','society_phase':runtime.get('phase'),'substantive_core_deliberation':True,'core_notes_present':sorted(notes.keys()),'foreground_research':deliberation,'client_message_id':mid}; _finish_message(mid,profile,result); return result
