...
9. Android release/UI hardening — **IMPLEMENTED; consolidated CI GREEN AT LAST OBSERVATION, LIVE DEVICE VALIDATION STILL REQUIRED.** v0.70 client baseline, device-local system/light/dark themes, custom accent/user-message/surface colours, automatic readable contrast, native artifact Download/Export and Share, and UI composition regressions are implemented. The authoritative APK build and release checkpoint use one `tools/compose_android_phase3.py` entrypoint. Historical patch modules remain as tested components, but workflows no longer duplicate the fragile ordered patch chain. The composer owns ordering and verifies required postconditions before Gradle runs.
10. Phase 3 release checkpoint — **IMPLEMENTED AS A RELEASE GATE; CI RESULT PENDING.** `JANUS_PHASE3_RELEASE_CHECKPOINT.md` freezes the Android/server v0.70 product surface. `.github/workflows/phase3-release-checkpoint.yml` runs server architecture/security/product regressions, Android Phase 3 product regressions, the authoritative composer, frozen UI/native/version assertions and a clean Gradle APK build, then publishes a release-candidate APK artifact with build identity. Treat the checkpoint as release-valid only after both release-gate jobs are green and the resulting APK is smoke-tested on a device.
...
`tools/compose_android_phase3.py` is the authoritative product composition entrypoint used by the APK workflow and release checkpoint. It applies the tested feature modules in one declared order and verifies key UI/native/version markers after composition. This reduces workflow drift without deleting the individual patch modules, so regressions remain attributable and fixes can still be made forward without reverting newer features.

## Phase 3 release boundary
The release checkpoint is a gate, not an automatic declaration of success. A green checkpoint means the repository's frozen v0.70 server/client contracts compose and build together. Final release acceptance still requires installing that release-candidate APK on a real device and smoke-testing sign-in, Chat, attachments, artifacts/export/share, Research Workspace, Maintenance, Background Research, Compatibility and theme persistence against the deployed server.
