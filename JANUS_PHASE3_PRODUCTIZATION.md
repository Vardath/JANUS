...
4. Android generated-artifact workflow — **IMPLEMENTED IN CLIENT BUILD PATH; CI/live validation pending.** Android Options exposes account-bound continuity reports and research digests, lists existing JANUS artifacts, opens provenance/details, can attach generated artifact files back into Chat for grounded discussion, and now exposes native Android Download/Export and Share controls.
...
8. Protocol/capability negotiation — **IMPLEMENTED IN SERVER + CLIENT BUILD PATH; primary Android/build checks green, one unrelated workspace run was queued at last observation.** The public `/protocol/capabilities` endpoint reports protocol compatibility, deployed commit identity, minimum/recommended Android client version, actual route-backed feature availability and protected safety boundaries. Android refreshes this capability map after sign-in and exposes a Compatibility screen; optional product buttons are disabled when the deployed server does not advertise the required feature rather than pretending support exists.
9. Android release/UI hardening — **IN PROGRESS.** v0.70 client baseline created. Device-local theme controls are implemented for system/light/dark appearance plus custom accent, user-message and surface colours with automatic readable accent contrast. Native artifact Download/Export now uses Android's document destination picker and authenticated account-bound file download; native Share uses a private cache FileProvider and Android share sheet. Dedicated UI composition regressions cover these controls. Remaining Step 9 work: further patch-chain reduction and final UI/live regression validation.
