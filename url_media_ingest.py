...
MAX_URLS=max(1,min(4,int(os.environ.get('JANUS_URL_MAX_PER_TURN','3'))))
MAX_CHARS=max(2000,min(24000,int(os.environ.get('JANUS_URL_MAX_CHARS','12000'))))
...
def _canonical(url):
    p=urlparse(url.strip()); host=p.netloc.lower().removeprefix('www.')
    if host in {'youtu.be','youtube.com','m.youtube.com'}:
        vid=''
        if host=='youtu.be': vid=p.path.strip('/').split('/')[0]
        elif p.path=='/watch': vid=parse_qs(p.query).get('v',[''])[0]
        elif p.path.startswith('/shorts/') or p.path.startswith('/live/'): vid=p.path.split('/')[2]
...
def _fetch_youtube(url):
    ...
    title='YouTube video'; text=''; available=False; reason='transcript unavailable'
    try:
        from youtube_transcript_api import YouTubeTranscriptApi
        api=YouTubeTranscriptApi()
        try: rows=api.fetch(vid)
        except Exception: rows=YouTubeTranscriptApi.get_transcript(vid)
        chunks=[]
        for r in rows:
            t=getattr(r,'text',None) if not isinstance(r,dict) else r.get('text')
            if t: chunks.append(str(t))
        text=' '.join(chunks); available=bool(text); reason='' if available else reason
    except Exception as exc: reason=f'transcript unavailable: {type(exc).__name__}'
    try:
...
def _fetch_web(url):
    r=requests.get(url,timeout=10,allow_redirects=True,headers={'User-Agent':'Mozilla/5.0 JANUSResearch/1.0','Accept':'text/html,text/plain,application/xhtml+xml'})
    ...
    if 'html' in ctype:
        ...
        m=re.search(r'<title[^>]*>(.*?)</title>',raw,re.I|re.S); title=html.unescape(re.sub('<[^>]+>',' ',m.group(1))).strip() if m else urlparse(url).netloc
    else: text=raw; title=urlparse(url).netloc
    text=' '.join(html.unescape(text).split())
    return {'kind':'web','title':title[:300],'content':text[:MAX_CHARS],'transcript_available':False,'note':''}
def ingest(profile,url):
    ...
    except Exception as exc: result={'kind':'web','title':urlparse(canonical).netloc,'content':'','transcript_available':False,'note':f'retrieval failed: {type(exc).__name__}'}
    ...
    with _db() as c:
        c.execute('INSERT OR REPLACE INTO janus_url_cache(profile_id,canonical_url,kind,title,content,transcript_available,provenance_json,fetched_at) VALUES(?,?,?,?,?,?,?,?)',(profile,canonical,result['kind'],result['title'],result['content'],1 if result['transcript_available'] else 0,json.dumps(prov),time.time())); c.commit()
...
def install(app, curiosity_module):
    ...
    def wrapped(profile,message):
        ...
        for u in urls[:MAX_URLS]:
            ...
            notes.append(f"SOURCE URL: {item.get('canonical_url',u)}\nTITLE: {item.get('title','')}\nSTATUS: {status}\nPROVENANCE: {json.dumps(item.get('provenance',{}),ensure_ascii=False)}\nRETRIEVED MATERIAL: {item.get('content') or ''}")
        enriched=str(message or '')
        if notes: enriched += '\n\nDIRECT URL MATERIAL (treat as source evidence; never invent missing transcript text):\n'+'\n\n'.join(notes)
    ...
    def research_capabilities():
        return {'web_search':bool(getattr(curiosity_module,'ENABLED',False)),'direct_url_ingestion':True,'youtube_transcript_attempt':True,'youtube_channel_enumeration':'bounded via web search; direct channel crawling disabled','transcript_policy':'availability is per-video; missing transcripts are never fabricated','url_cache':True,'account_isolated_cache':True}
