...
def pulse(profile:str)->dict:
    ...
    a=rows[seed%len(rows)]; b=rows[(seed//max(1,len(rows))+max(1,len(rows)//2))%len(rows)] if len(rows)>1 else a
    ...
    _event(profile,"hive_core_burst",json.dumps({"pulse":count,"processed":burst.get("processed",[]),"burst_count":burst.get("burst_count")},ensure_ascii=False))
    ...
    return {"ok":True,"pulse":count,"memory_ids":[a['id'],b['id']],"burst":burst,"message":message}
...
async def _worker():
    ...
    while True:
        ...
        if os.environ.get("JANUS_PAID_BACKGROUND_REFLECTION","1")=="1" and now-last_paid>=paid_seconds:
            for profile in profiles:
                await _language_reflection(profile); await asyncio.sleep(0.25)
            last_paid=now
