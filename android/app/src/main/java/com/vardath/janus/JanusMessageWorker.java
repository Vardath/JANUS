...
public class JanusMessageWorker extends Worker {
    ...
    @NonNull @Override public Result doWork() {
        String profile = getApplicationContext().getSharedPreferences("janus", Context.MODE_PRIVATE).getString("profile_id", "");
        if (profile == null || profile.isEmpty()) return Result.success();
        try {
            String q = URLEncoder.encode(profile, StandardCharsets.UTF_8.toString());
            HttpURLConnection c = (HttpURLConnection) new URL(MainActivity.SERVER + "/desktop/messages?limit=20&username=" + q).openConnection();
            c.setRequestMethod("GET");
            c.setConnectTimeout(20000);
            c.setReadTimeout(30000);
            c.setRequestProperty("Accept", "application/json");
            if (c.getResponseCode() >= 400) return Result.retry();
            ...
            long newest = getApplicationContext().getSharedPreferences("janus", Context.MODE_PRIVATE).getLong("last_notified_message", 0);
            ...
            if (candidate != null) {
                ...
                getApplicationContext().getSharedPreferences("janus", Context.MODE_PRIVATE).edit().putLong("last_notified_message", candidateId).apply();
            }
