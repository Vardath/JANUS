    ...
    if event_type == "proactive_message":
        try:
            ...
            if isinstance(payload, dict):
                ...
                if text: return message_type, text, source
        except Exception:
            pass
...
def _message_rows(profile: str, limit: int = 50, include_dismissed: bool = False):
    ...
    try:
        ...
        for row in rows:
            ...
            item["message_type"] = message_type; item["detail"] = text; item["source"] = source
            items.append(item)
...
def desktop_runtime_cores(username: str | None = Query(default=None)):
    ...
    return {'profile': username or 'unspecified','architecture': '7 -> 3 -> 1','runtime': status,'paid_background_api_enabled': os.environ.get('JANUS_SELF_EVALUATION', '0') == '1','note': 'Seven local cores communicate during wake windows and consolidate during sleep. The local cycle makes no external model/API calls.'}
...
def desktop_messages(username: str = Query(...), limit: int = Query(default=50, ge=1, le=100), include_dismissed: bool = Query(default=False)):
    ...
    return {'profile': username,'items': items,'unread': sum(1 for x in items if x['state'] == 'unread'),'message_types': ['Question', 'Observation', 'Memory', 'Follow-up'],'purpose': "JANUS's persistent outbox to the user.",'runtime_action': True}
...
def desktop_message_state(event_id: int, payload: dict[str, Any]):
    ...
    try:
        ...
        if not exists: raise HTTPException(404, 'message not found')
